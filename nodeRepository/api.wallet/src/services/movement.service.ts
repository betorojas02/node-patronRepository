...
export class MovementService {
    constructor(private readonly movementRepository: MovementRepository, private readonly balanceRepository: BalanceRepository) {
    ...
    public async store(entry: MovementCreateDto): Promise<void> {
        ...
        if (entry.type === MovementType.income) {
            ...
            throw new ApplicationException("Invalid moment type supplied")
        }
    }
    ...
    private async income (entry: MovementCreateDto, balance: Balance | null) {
        ...
        this.movementRepository.store(entry as Movement);
    }
